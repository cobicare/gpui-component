...
mod tests {
    ...
    #[gpui::test]
    fn test_completion_text_edit_replaces_typed_prefix(cx: &mut TestAppContext) {
        use crate::input::popovers::CompletionMenu;
        use lsp_types::{CompletionItem, CompletionResponse, CompletionTextEdit, TextEdit};
        use std::rc::Rc;

        struct PrefixProvider;
        impl crate::input::CompletionProvider for PrefixProvider {
            fn completions(
                &self,
                rope: &crate::input::Rope,
                offset: usize,
                _trigger: lsp_types::CompletionContext,
                _window: &mut gpui::Window,
                _cx: &mut gpui::Context<crate::input::InputState>,
            ) -> gpui::Task<anyhow::Result<CompletionResponse>> {
                use crate::input::RopeExt as _;
                let text = rope.slice(..offset).to_string();
            ...
            fn is_completion_trigger(
                ...
                !new_text.is_empty() && new_text.chars().all(|ch| ch.is_ascii_alphanumeric())
            }
        }
            ...
            cx.open_window(Default::default(), |window, cx| {
                ...
                input = Some(cx.new(|cx| {
                    let mut state = InputState::new(window, cx)
                        .code_editor("sql")
                        .line_number(false);
                    state.lsp.completion_provider = Some(Rc::new(PrefixProvider));
                    state
                }));
                cx.new(|cx| crate::Root::new(input.clone().unwrap(), window, cx))
            })
        ...
        cx.simulate_keystrokes("s e l e");
        cx.run_until_parked();
        // Accept the highlighted completion.
        cx.simulate_keystrokes("enter");
        cx.run_until_parked();
        let value = cx.update(|_, cx| input.read_with(cx, |state, _| state.value().to_string()));
        assert_eq!(value, "select", "accepting must replace the typed prefix");
        let _ = CompletionMenu::is_open; // silence unused import paths on some configs
    }
